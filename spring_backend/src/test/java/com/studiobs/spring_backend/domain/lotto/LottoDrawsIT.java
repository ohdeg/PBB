...
class LottoDrawsIT extends AbstractIntegrationTest {
    ...
    @Test
    void listDraws_isPublic() throws Exception {
        mockMvc.perform(get("/api/v1/lotto/draws"))
                .andExpect(status().isOk());
    }

    @Test
    void upsertDraw_forbidden_forNonDev() throws Exception {
        String email = "lotto_" + System.nanoTime() + "@example.com";
        String nickname = "lotto" + (System.nanoTime() % 1_000_000);
        String accessToken = signupAndLogin(email, nickname);

        mockMvc.perform(put("/api/v1/lotto/draws")
