...
- 메뉴 등록: 재고와 같이 모달 한 장. 카테고리(선택 또는 직접 입력) + 제목 + **사진(선택)** + 노트. 카테고리가 없으면 만든 뒤 레시피를 붙인다. 빈 화면 「메뉴 추가」·목록 「새로 추가」·레시피 0 「첫 레시피 적기」가 같은 모달을 연다.
- 레시피 사진: nullable. R2 공개 URL만 DB(`brew_recipes.image_url`)에 저장. 업로드 전후 긴 변 1280 JPEG. 레시피 추가·수정에서 넣고, 목록·상세에 보인다. 데모는 data URL. 카테고리에는 사진을 두지 않는다.
...
단위·농도·타이머는 프론트 계산. 호출벨 멘트·속도·음높이·벨/멘트 음량은 가게에 저장. 탭 이동 중에도 타이머는 모듈 상태로 유지.
...
- **호출벨**: 번호는 매번 입력(엔터=호출). 항상 띵–동 후 `{번호} {멘트}` TTS. 멘트는 비우면 null(번호만). 속도(0.5–2)·음높이(0–2)·벨/멘트 음량(0–1)은 도구 탭 호출벨의 **설정** 모달에서만 조절하고, 조작 후 약 0.4초 뒤 가게에 저장. **저장**은 멘트만. `brew_stores.call_bell_phrase` JSON `{phrase,rate,pitch,chimeVolume,speechVolume}`. 구버전 값은 멘트 문자열. TTS는 브라우저 언어. `PUT /api/v1/veveno/stores/{id}/call-bell` (owner·구독자). 번호는 저장하지 않음. 체험은 이 기기 `localStorage`. 가게 설정 탭에는 없음.
...
- 재고 행: `−`/`+`는 1씩. 숫자는 `12개`처럼 단위와 함께 표시. **숫자 탭** → 입고 수량만큼 더함. 목록 헤더 **편집** → 행 `⋯`에서 이름·카테고리·수량·단위·경고선·**사진(선택)** 수정·삭제. **발주 링크는 사장만**(직원 GET 응답에 `orderUrl` 없음, PATCH로도 못 넣음)
- 재고 사진: nullable. R2 URL만 `brew_store_stocks.image_url`. 같은 리사이즈. POS 수량 PATCH는 `imageUrl` 생략 → 유지. 변경 모달에 썸네일.
